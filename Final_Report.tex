...
For the first phase, I started with data generation and Least Squares (LS) baselines. I simulated two spring variants (Fig. \ref{springs profile}). The original system with deadzone threshold $\delta{=}1.0$, and a shrunk variant with $\delta{=}0.25$ (as suggested by Prof. Caines), where the spring enters a steeper linear region at much smaller displacements. For each variant, I generated a training trajectory (with seed~42, Fig. \ref{original non lin springs with delta 1}) and a held-out test trajectory
