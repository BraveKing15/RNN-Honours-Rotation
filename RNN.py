...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        # These need to be wrapped in nn.Parameter so PyTorch knows to optimize them!
        self.W_hh = nn.Parameter(torch.randn(hidden_size, hidden_size) * 0.1)
        # TODO: Create W_uh (shape: input_size x hidden_size)
        self.W_uh = nn.Parameter(torch.randn(input_size, hidden_size) * 0.1)
        # TODO: Create bias b_h (shape: hidden_size, initialize with zeros)
        self.b_h = nn.Parameter(torch.zeros(hidden_size))
        
        # State Prediction Weights
        # TODO: Create W_hx (shape: hidden_size x output_size)
        self.W_hx = nn.Parameter(torch.randn(hidden_size, output_size) * 0.1)
        # TODO: Create bias b_x (shape: output_size, initialize with zeros)
